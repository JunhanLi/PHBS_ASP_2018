...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        vol_kk = cov_m.diagonal()
        vol_kk = np.tile(vol_kk,(n_samples,1)).transpose() 
...
def basket_price_norm_analytic(
    ...
    spot_basket = weights @ spot

    cov_m = vol * cor_m * vol[:,None]
    vol_basket = np.sqrt(weights @ cov_m @ weights.transpose())

    price = normal_price(strike, spot_basket, vol_basket, texp, intr=0.0, divr=0.0, cp_sign=1)
